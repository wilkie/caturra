...
fn find_main(sources: &[jvmjs_compiler::SourceFile]) -> Option<String> {
    for f in sources {
...
fn main() {
    ...
    for part in buf.split('\u{1e}') {
        if let Some(r) = part.strip_prefix("F:") {
            if let Some((n, t)) = r.split_once('\u{1f}') {
                files.push(jvmjs_compiler::SourceFile {
                    path: n.into(),
                    text: t.into(),
                });
            }
    ...
    let compiled = jvmjs_compiler::compile(&files);
    if !compiled.success() {
        let err = compiled
            .diagnostics
            .iter()
            .find(|d| matches!(d.severity, jvmjs_compiler::diagnostics::Severity::Error));
    ...
    let mut vfs = jvmjs_vm::VirtualFileSystem::new();
    if !grid.is_empty() {
    ...
    let mut console = jvmjs_vm::BufferedConsole::with_input(input);
    let options = jvmjs_vm::VmOptions {
        max_instructions: 20_000_000,
        ..jvmjs_vm::VmOptions::default()
    };
    let mut vm = jvmjs_vm::Vm::new(options, &mut vfs, &mut console);
