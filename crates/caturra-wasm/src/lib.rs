...
use caturra_compiler::{Severity, SourceFile};
use caturra_vm::{ConsoleIo, VirtualFileSystem, Vm, VmOptions};
use serde::{Deserialize, Serialize};
use wasm_bindgen::prelude::*;

/// Mirrors [`caturra_compiler::SourceFile`] across the JS boundary.
...
impl JsDebugHost<'_> {
    ...
    fn evaluate_watches(
        &self,
        snapshot: &caturra_vm::DebugSnapshot,
        watch: &mut dyn caturra_vm::WatchEvaluator,
...
fn evaluate_watch(
    ...
    let usable: Vec<&caturra_vm::LocalSnapshot> = frame
        .locals
        .iter()
    ...
    let watch_source = format!(
        "class __CaturraWatch {{\n    static String __eval({params}) \
         {{\n        return \"\" + ({expression});\n    }}\n}}\n"
    );
    let mut all = sources.to_vec();
    all.push(SourceFile {
        path: String::from("__CaturraWatch.java"),
        text: watch_source,
    });

    let compilation = caturra_compiler::compile(&all);
...
impl caturra_vm::DebugHost for JsDebugHost<'_> {
    fn on_pause(
        &mut self,
        snapshot: &caturra_vm::DebugSnapshot,
        watch: &mut dyn caturra_vm::WatchEvaluator,
    ) -> caturra_vm::DebugControl {
...
fn snapshot_to_json(
    ...
    let js = JsSnapshot {
        reason: match snapshot.reason {
            caturra_vm::PauseReason::Breakpoint => "breakpoint",
...
struct ParsedControl {
    control: caturra_vm::DebugControl,
    watches: Option<Vec<String>>,
}

fn parse_breakpoints(json: &str) -> Vec<caturra_vm::Breakpoint> {
    serde_json::from_str::<Vec<JsBreakpoint>>(json)
        .unwrap_or_default()
        .into_iter()
        .map(|b| caturra_vm::Breakpoint {
            file: b.file,
            line: b.line,
        })
        .collect()
...
fn parse_debug_control(json: &str) -> (ParsedControl, bool) {
    ...
    let control = caturra_vm::DebugControl {
        command: match command.as_str() {
            // A refresh round never reaches the VM; Continue is a
            // placeholder for the loop.
            "continue" | "refresh" => caturra_vm::DebugCommand::Continue,
            "stepOver" => caturra_vm::DebugCommand::StepOver,
            "stepInto" => caturra_vm::DebugCommand::StepInto,
            "stepOut" => caturra_vm::DebugCommand::StepOut,
            _ => caturra_vm::DebugCommand::Terminate,
        },
...
pub struct JvmSession {
    ...
    classes: Vec<caturra_compiler::CompiledClass>,
}
...
impl JvmSession {
    ...
    pub fn run(
        ...
        let result = match vm.run_main(main_class, &args) {
            Ok(caturra_vm::vm::ExitStatus::Completed) => JsRunResult {
                status: "completed",
                exit_code: 0,
                error: None,
            },
            Ok(caturra_vm::vm::ExitStatus::Exited(code)) => JsRunResult {
                status: "exited",
                exit_code: code,
                error: None,
            },
    ...
    pub fn run_debug(
        ...
        let result = match vm.run_main_debug(main_class, &args, &breakpoints, &mut host) {
            ...
            Err(caturra_vm::VmError::Stopped) => JsRunResult {
                status: "stopped",
                exit_code: 0,
                error: None,
            },
...
/// Version of the caturra engine, for display in the host page.
#[wasm_bindgen]
#[must_use]
