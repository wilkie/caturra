/*!
 * coi-serviceworker v0.1.7 — Cross-Origin Isolation via a service worker.
 * Based on https://github.com/gzuidhof/coi-serviceworker (MIT, Stefan Gzuidhof).
 *
 * GitHub Pages (and most static hosts) cannot send COOP/COEP response headers,
 * which SharedArrayBuffer requires. This worker re-serves same-origin responses
 * with those headers so `crossOriginIsolated` becomes true — caturra needs it
 * for blocking stdin (Scanner). The dev/preview servers set the headers
 * directly, so index.html only loads this off-localhost.
 */
/* eslint-disable */
...
if (typeof window === 'undefined') {
  ...
  (() => {
    ...
    // If already isolated (dev server headers), nothing to do.
