...
Emits two files:
  - src/csa-levels.ts        COMMITTED — student start sources + data files only.
  - src/csa-validators.local.ts  GIT-IGNORED — the teacher-authored validators
    (from each level's plaintext validation/ dir). We never commit validation or
    solution sources, and we never read Code.org's encrypted_validation cipher.
The playground merges the local overlay when present (so the Test button works
in a dev checkout) and degrades to "no tests" without it.
...
def load_level(key):
    ...
    if not student:
        return None
    return vm, (grid or ""), student, validators, start_datafiles(key)
    ...
    for sl in d["script_levels"]:
        ...
        for key in (sl.get("level_keys") or []):
            ...
            for let, tkey, dn in targets:
                ...
                if not got:
                    continue
                vm, grid, student, validators, data = got
                ...
                levels.append({
                    ...
                    "files": student, "validation": validators, "data": data,
                })
...
def build_frq_unit():
    ...
    for key in keys:
        ...
        levels.append({
            ...
            "view": "console", "grid": "", "files": student, "validation": validators, "data": data,
        })
...
# csa-levels.ts — COMMITTED. Student start sources + data files only;
# validationFiles is always empty here (validators are the local overlay).
out = [
    "// AUTO-GENERATED by apps/playground/scripts/generate-csa-levels.py — do not edit.",
    "// Student start sources and data files only. Teacher-authored validators are",
    "// NOT committed: they live in the git-ignored csa-validators.local.ts overlay,",
    "// generated locally from the artifacts' validation/ dirs and merged at load.",
    "",
    "export interface CsaLevelFile {", "  path: string;", "  text: string;", "}", "",
    "export interface CsaLevel {", "  name: string;", "  lesson: string;",
    "  view: 'neighborhood' | 'console';", "  grid: string;", "  files: CsaLevelFile[];",
    "  /** Teacher-authored JUnit validators, filled by the local overlay if present. */",
    "  validationFiles: CsaLevelFile[];",
    "  /** Data files (e.g. .txt read via FileReader) written to the VFS before a run. */",
    "  dataFiles: CsaLevelFile[];", "}", "",
    "export interface CsaUnit {", "  name: string;", "  levels: CsaLevel[];", "}", "",
    "export const CSA_UNITS: CsaUnit[] = [",
]
for u in units:
    out += ["  {", f"    name: {js(u['name'])},", "    levels: ["]
    for l in u["levels"]:
        out += [
            "      {", f"        name: {js(l['name'])},", f"        lesson: {js(l['lesson'])},",
            f"        view: {js(l['view'])},", f"        grid: {json.dumps(l['grid'])},", "        files: [",
        ]
        out += emit_files(l["files"], "          ")
        out += ["        ],", "        validationFiles: [],", "        dataFiles: ["]
        out += emit_files(l.get("data", []), "          ")
        out += ["        ],", "      },"]
    out += ["    ],", "  },"]
out += ["];", ""]
open(os.path.join(SRC, "csa-levels.ts"), "w").write("\n".join(out))

# csa-validators.local.ts — GIT-IGNORED. Validators keyed by "unit:level"
# index; the playground merges them into CSA_UNITS when the file exists.
vout = [
    "// AUTO-GENERATED, GIT-IGNORED — teacher-authored validators for the Test",
    "// button, generated locally from the artifacts' validation/ dirs. Never",
    "// commit this file (validation sources are not redistributed).",
    "",
    "import type { CsaLevelFile } from './csa-levels.js';",
    "",
    "export const VALIDATORS: Record<string, CsaLevelFile[]> = {",
]
for ui, u in enumerate(units):
    for li, l in enumerate(u["levels"]):
        if not l["validation"]:
            continue
        vout += [f"  '{ui}:{li}': ["]
        vout += emit_files(l["validation"], "    ")
        vout += ["  ],"]
vout += ["};", ""]
open(os.path.join(SRC, "csa-validators.local.ts"), "w").write("\n".join(vout))
