...
# validationFiles/solutionFiles are always empty here (the local overlay fills
# them); validators and solutions are never committed.
out = [
    "// AUTO-GENERATED by apps/playground/scripts/generate-csa-levels.py — do not edit.",
    "// Student start sources and data files only. Teacher-authored validators and",
    "// solutions are NOT committed: they live in the git-ignored",
    "// csa-overlay.local.ts, generated locally and merged at load.",
    "",
    ...
    "  /** The complete solution, filled by the local overlay if present (Solve button). */",
...
for u in units:
    ...
    for l in u["levels"]:
        ...
        out += ["        ],", "        validationFiles: [],", "        solutionFiles: [],", "        dataFiles: ["]
...
# csa-overlay.local.ts — GIT-IGNORED. Teacher-authored validators (Test button)
# and complete solutions (Solve button), keyed by "unit:level" index and merged
# into CSA_UNITS when the file exists. Never commit (not redistributable).
vout = [
    "// AUTO-GENERATED, GIT-IGNORED — teacher-authored validators and solutions,",
    "// generated locally from the artifacts. Never commit this file.",
    "",
    "import type { CsaLevelFile } from './csa-levels.js';",
    "",
]
vout += emit_map("VALIDATORS", "validation")
vout += emit_map("SOLUTIONS", "solution")
open(os.path.join(SRC, "csa-overlay.local.ts"), "w").write("\n".join(vout))
