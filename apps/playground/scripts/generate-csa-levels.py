...
# csa-validators.ts — COMMITTED. Teacher-authored JUnit validators (Test button),
# keyed by "unit:level" index and merged into CSA_UNITS. Committed so the hosted
# playground (GitHub Pages) can run the Test button.
val = [
    "// AUTO-GENERATED by apps/playground/scripts/generate-csa-levels.py — do not edit.",
    "// Teacher-authored JUnit validators (Test button), keyed by unit:level index.",
    "",
    "import type { CsaLevelFile } from './csa-levels.js';",
    "",
]
val += emit_map("VALIDATORS", "validation")
open(os.path.join(SRC, "csa-validators.ts"), "w").write("\n".join(val))
